...
// Stub the one runtime value the consult uses from the public API. Spread the
// real module so types / RiskLevel / etc. stay intact for the rest of the graph.
...
const { consultAdvisor } = await import("../src/consult.js");
...
describe("consultAdvisor", () => {
  test("routes through the inference call site, tools off, capped, returns advice", async () => {
    ...
    // The model sees the sanitized transcript (thinking + pending tool_use
    // stripped) plus the appended request turn carrying the word-limit nudge.
    ...
    const lastText = (sent[sent.length - 1].content[0] as { text: string }).text;
